...
$pdf->Cell(190, 8, 'Sales Report', 0, 0, 'C');
$pdf->SetXY(12, 26);
$pdf->SetFont('helvetica', '', 11.5, '', true);
if (isset($type) && isset($username))
{
	$pdf->Cell(10, 8, 'User Type : ' . $type, 0, 0, 'L');
	$pdf->SetXY(12, 32);
	$pdf->Cell(10, 8, 'User Name : ' . $username, 0, 0, 'L');
	$pdf->SetXY(12, 39);
} else {
	$pdf->SetXY(12, 30);
}
...
$htmlData = '
			...
			<table width="100%" style="padding:10px;border: 1px solid #7c9bbd;">
				<tr style="background-color: #7c9bbd;">
					...
					<th style="border-right: 1px solid #7c9bbd;">Status</th>
				</tr>
			';
foreach ($data as $datum)
{
	$htmlData .= '<tr style="background-color: #f8f8f8;">
					<td>'. $datum['orderId'] .'</td>
					<td>'. $datum['name'] .'</td>
					<td>'. $datum['totalItems'] .'</td>
					<td>'. $datum['date'] .'</td>
					<td>'. $datum['totalAmount'] .'</td>
					<td>'. $datum['payMethod'] .'</td>
					<td>'. $datum['status'] .'</td>
				</tr>';
}
$htmlData .= '</table>';
