...
{
	$html .= '<option value="'. $item['stateId'] .'">'. $item['stateName'] .'</option>';
}
?>
<!doctype html>
<html lang="en">
	...
	<div class="app-container"><!-- Horizontal navbar -->
		...
		<main class="main-content">
			...
			<div class="main-content-wrap">
				<header class="page-header"><h1 class="page-title">Cities</h1></header>
...
<div aria-hidden="true" class="modal fade" id="add-state" role="dialog" tabindex="-1">
	<div class="modal-dialog modal-dialog-centered  modal-dialog-scrollable">
		<div class="modal-content">
			<div class="modal-header"><h5 class="modal-title">Add new City</h5></div>
			<div class="modal-body">
				<form id="add-state-parsley">
					<div class="form-group"><input class="form-control" id="cityName" placeholder="City Name" type="text" required></div>
					<div class="form-group">
						<select id="newStateId" class="selectpicker" required data-parsley-min="1" data-parsley-min-message="This value is required.">
			...
			<div class="modal-footer d-block">
				<div class="actions justify-content-between">
					...
					<button class="btn btn-info" type="button" id="addS">Add City</button>
				</div>
			</div>
		</div>
	</div>
...
<div aria-hidden="true" class="modal fade" id="edit-state" role="dialog" tabindex="-1">
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
		<div class="modal-content">
			<div class="modal-header"><h5 class="modal-title">Edit City</h5></div>
			<div class="modal-body">
				<div class="form-group">
					<label for="">City Id</label>
					<span class="form-control" id="editcityId"></span>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<select id="stateId" class="selectpicker" >
						<option value="0">Select</option>
						<?= $html?>
					</select>
				</div>
...
<script>
	...
	async function fetchStateList() {
		const response = await fetch('<?= base_url()?>admin/city/fetchCityList');
		...
		for (let i = 0; i < myJson.length; i++) {
			...
			html += '<tr>\n' +
				'<td>'+ myJson[i].cityId+'</td>\n' +
				'<td>'+ myJson[i].cityName+'</td>\n' +
				'<td>'+ myJson[i].stateName+'</td>\n' +
				'<td>\n' +
				'<div class="actions">\n' +
				'<button class="btn btn-info btn-sm btn-square rounded-pill edit-state" data-cityid="'+ myJson[i].cityId+'" data-target="#edit-state" data-toggle="modal"><span\n' +
	...
	async function fetchState(id) {
		const response = await fetch('<?= base_url()?>admin/city/fetchCity/'+ id);
		const myJson = await response.json();

		$("#editcityId").text(myJson.cityId);
		$("#editname").val(myJson.cityName);
		$("#stateId>option[value='" + myJson.stateId + "']").prop("selected" , true).change();
		$('.selectpicker').selectpicker('refresh');
	}

	$("#editDoc").on('click', function () {
		const stateId = $("#stateId").val();
		const cityId = $("#editcityId").text();
		const cityName = $("#editname").val();

		$.ajax({
			url: '<?= base_url()?>admin/city/updateCity' ,
			method: 'post',
			data: {
				stateId:stateId,
				cityId:cityId,
	...
	$("#addS").on('click', function () {
		...
		if(pars.isValid()) {
			...
			const newStateId = $("#newStateId").val();

			$.ajax({
				url: '<?= base_url()?>admin/city/addCity' ,
				method: 'post' ,
				data: {
					cityName: cityName ,
					stateId: newStateId ,
